 ...
 * Used for dynamic page refresh with AJAX
*/

function retrofitAttr_jscript(equipType) {
    $.ajax({
        type: "GET",
        url: 'retrofitAttr',  // URL to your view that serves new info
...
function getKioskForm_jscript(formType) {
    $.ajax({
        type: "GET",
        url: 'getKioskModelForm', // URL to your view that serves new info
...
function createKioskModel_jscript(kioskType, ctsDivision) {
    $.ajax({
        type: "GET",
        url: 'createKioskModel', // URL to your view that serves new info
...
///// Production Release - Javascript functions for partial page reloads
...
function addEquipList(currentEquip, newComp, nextComp, kioskType) {
    ...
    //compDict[newComp].push(currentEquip);
    //compDict.newComp.push(currentEquip);

    //document.write(compDict['2']);
...
function editExistSubequip(equipID, compID, jobNumber) {
    ...
    $.ajax({
        ...
        data: { 'equip': equipID, 'comp': compID, 'jobNumber': jobNumber }
    })
        .done(function (response) {
...
///// End Production Release Javascript functions

/////

///// Update Database - Javascript functions for partial page reloads

function getDatabaseForm_jscript(formType) {
    $.ajax({
        type: "GET",
        url: 'getDatabaseForm', // URL to your view that serves new info
        data: { 'formType': formType }
    })
        .done(function (response) {
...
///// End Update Database Javascript functions