...
///// End Search Recore functions

/////

///// Retrofit Kiosk functions

function retrofitSubequip_jscript(equipType) {
    $.ajax({
        type: "GET",
        url: 'retrofitSubequip',
        data: { 'equip': equipType }
    })
        .done(function (response) {
            $('#retrofitSubequip_JS').replaceWith(response);
        });
}

///// End Retrofit Kiosk functions
...
///// MT New Kiosk functions

function mtNewKioskCheckbox(checkbox, subEquipDiv) {
    if (checkbox.checked == true) {
        currentEquip = checkbox.value;
        getSubEquip_jscript(currentEquip, subEquipDiv)
    }
        ...
        resetProdRlse_jscript(subEquipDiv)
    }
}

///// End MT New Kiosk functions
